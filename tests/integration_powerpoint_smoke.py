...
def create_sample_presentation(path: Path) -> None:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="gesture-ppt-integration-", dir=base) as temporary:
        root = Path(temporary)
        source = root / "中文 路径 集成测试.pptx"
        create_sample_presentation(source)
        ...
        print(f"COM_EXPORT_OK pages={first.project.slide_count} cache_hit={second.cache_hit}")
    return 0
